...
async def _health_check_batch(session: aiohttp.ClientSession, hosts: list[dict], concurrency: int = 64, trace_id: str = "") -> list[dict]:
    ...
    logger.info(f"🔍 {tag}[健康检查] {len(valid)}/{len(hosts)} 个有效")
    return valid


async def query_geoip(session: aiohttp.ClientSession, ip: str, trace_id: str = "") -> dict:
    tag = f"[trace:{trace_id}] " if trace_id else ""
        ...
        async with session.get(url, timeout=5) as resp:
            if resp.status == 200:
                ...
                if data.get("status") == "fail":
                    logger.info(f"⚠️ {tag}[geoip] {ip} API 失败: {data.get('message', 'unknown')}")
                    return {"region": "", "operator": "", "countryCode": ""}
                # 排除国外IP（只保留中国）
                country_code = data.get("countryCode", "")
                if country_code != "CN":
                    logger.info(f"🌍 {tag}[geoip] {ip} 国外IP({country_code})，已排除")
                ...
                logger.info(f"🌍 {tag}[geoip] {ip} -> {result}")
                return result
            else:
...
async def enrich_geo_batch(session: aiohttp.ClientSession, sources: list[dict], trace_id: str = "") -> list[dict]:
    ...
    sources = await _health_check_batch(session, sources, trace_id=trace_id)
    if not sources:
        return []
    ...
    for item in sources:
        ...
        geo = await query_geoip(session, ip_part, trace_id=trace_id)
        # 排除国外IP
    ...
    logger.info(f"🌍 {tag}[geoip富化] 共 {len(sources)} 条, 缓存命中 {cache_hit_count} 条, API查询 {queried_count} 条, 已有geo跳过 {skipped_count} 条, 国外IP排除 {foreign_count} 条")
    return enriched
